...
void polar_angle_tilt(const char* inFName, const char* outFName, const double polarAngleChange);
...
int main(int argc, char** argv){
	if(argc != 4){
    	std::cout << "**Not enough arguments. Remember to put input file, output file, and polar angle change (in degrees)**" << std::endl;
    	return 1;
  	}

  	polar_angle_tilt(argv[1], argv[2], atof(argv[3]));
}
