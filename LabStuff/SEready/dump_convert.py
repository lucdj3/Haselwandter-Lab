...
def polar_change(in_fn, out_fn, polar_angle_change):
	...
	if(float(polar_angle_change) > tilt_size):
		...
		sproc.call("./polar_change " + in_fn + " " + rec_fn + str(0) + ".txt" + " " + str(tilt_size), shell=True)
		sproc.call(evolv_cmd + " " + rec_fn + str(0) + ".txt", shell=True, stdout = devnull)

		for i in range(loop_runs):
			sproc.call("./polar_change " + rec_fn + str(i) + ".txt.dmp" + " " + rec_fn + str(i+1) + ".txt" + " " + str(tilt_size), shell=True)
		...
		sproc.call("./polar_change " + rec_fn + str(loop_runs) + ".txt.dmp" + " " + out_fn + " " + str(final_change), shell=True)
		sproc.call("rm " + rec_fn + str(loop_runs) + ".txt", shell=True)
		sproc.call("rm " + rec_fn + str(loop_runs) + ".txt.dmp", shell=True)

	else:
		sproc.call("./polar_change " + in_fn + " " + out_fn + " " + polar_angle_change, shell=True)
...
if(len(sys.argv) != 4):
	print "Usage: \n\t"
	print sys.argv[0]+" [inputfile] [outputfile] [polar angle change]"
	exit()
else:
	polar_change(sys.argv[1], sys.argv[2], sys.argv[3])